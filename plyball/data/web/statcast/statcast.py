...
class StatCast(object):
    ...
    def large_request(self, start_date, end_date, detail, step, verbose):
        """
        ...
        while intermediate_dt <= end_date:
            ...
            data = self.__small_request(self.urls['search'].format(
            ...
            if 'error' in data.columns:
                ...
                while success == 0:
                    ...
                    if error_counter > 2:
                        ...
                        parameters['start_date'] = start_date.strftime('%Y-%m-%d')
                        parameters['end_date'] = intermediate_dt.strftime('%Y-%m-%d')
                        smaller_data_1 = self.__small_request(self.urls['search'].format(
                            '&'.join(['{}={}'.format(k, v) for k, v in parameters.items()])))
                        if 'error' in data.columns:
                            logger.warning(
